...
def register(request):
    ...
    if request.method == "POST":
        try:
            ...
            user.save()
            print(
                f"User created: {user.username}, {user.email}, active: {user.is_active}"
            )
            ...
            if email_sent:
                messages.success(
                    request,
                    _(
                        "注册成功！我们已向您的邮箱发送了验证链接，请点击链接激活您的账号。"
                    ),
                )
                ...
                messages.warning(
                    request,
                    _(
                        "邮件发送失败，但您的账号已创建。请联系管理员激活您的账号或稍后重试。"
                    ),
                )
                return redirect("users:register_done")
